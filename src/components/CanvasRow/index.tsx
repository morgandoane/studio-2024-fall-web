...
const CanvasRow: FC<CanvasRowProps> = ({
  balancers,
  maxEvents,
  maxDemand,
  maxSupply,
  ...
  useEffect(() => {
    ...
    const canvas = new P5((p5: P5) => {
      ...
      const multiBeatCanvas = new MultiBeatCanvas(
        p5,
        width,
        height,
        balancers,
  ...
  useEffect(() => {
    multiBeatCanvasRef.current?.setData(
      balancers,
      maxSupply,
      maxDemand,
      minSupply,
  ...
  }, [balancers, maxDemand, maxSupply, minDemand, minSupply]);
